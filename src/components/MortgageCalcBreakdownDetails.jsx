    ...
    return (
        <div className="flex flex-col w-full max-w-md p-4 bg-white rounded-lg shadow-md space-y-3 m-auto my-16">            
            ...
            <div className="flex justify-between items-center">
                ...
                <span className="font-medium text-gray-800">{valueFormatter(Math.round(monthlyInsurance))}</span>
            </div>
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-gray-800">{valueFormatter(Math.round(monthlyPropertyTax + monthlyHOAFees))}</span>
            </div>
            
            {/* Otros elementos de desglose */}
            <div className="flex justify-between">
                <span className="text-gray-600">Property Taxes:</span>
                <span className="font-medium text-gray-800">{valueFormatter(Math.round(monthlyPropertyTax))}</span>
            </div>
            <div className="flex justify-between">
                <span className="text-gray-600">HOA/Condo Fees:</span>
                <span className="font-medium text-gray-800">{valueFormatter(Math.round(monthlyHOAFees))}</span>
            </div>
            
            {/* Total de pagos mensuales */}
            <div className="flex justify-between border-t pt-2 mt-2">
                <span className="text-lg font-semibold">Total Monthly Payment:</span>
                <span className="text-lg font-semibold text-green-600">{valueFormatter(Math.round(totalMonthlyPayment))}</span>
            </div>
        </div>
    );
}
